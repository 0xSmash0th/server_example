...
(1: 연합, 2: 제국)',
  ...
  comment '유효 최대 행동력',
  activity_willbe_refreshed_at TIMESTAMP NULL DEFAULT NULL
...
',
  ...
  legion_contrib int
  comment '군단 공헌도',
  legion_joined_at TIMESTAMP NULL DEFAULT NULL
  comment '군단 가입 일시',
  legion_parted_at TIMESTAMP NULL DEFAULT NULL
  comment '군단 탈퇴 일시',
  ...
  officer_list_willbe_reset_at TIMESTAMP NULL DEFAULT NULL
  comment '장교 목록 리셋될 시간 (일정 시간이 지나면 자동 리셋해야 함)',
  tax_collectable_count int not null default 0
  comment '세금 징수 가능 회수',
  tax_timer_willbe_refreshed_at TIMESTAMP NULL DEFAULT NULL
  ...
  bld_cool_end_at TIMESTAMP NULL DEFAULT NULL
...
id:{level:n}, ...}',
  ...
  comment '장착된 훈장 아이디',
  badge_willbe_refreshed_at TIMESTAMP NULL DEFAULT NULL
...
',
  tutorial_list TEXT comment '완료되지 않은 튜토리얼 리스트. json array of tutorial_id',
  chat_ban_ids text
  ...
  mail_pulled_legion_at TIMESTAMP NULL DEFAULT NULL,
  mail_pulled_public_at TIMESTAMP NULL DEFAULT NULL,
...
5: dead',
  ...
  comment '고용된 시간',
  status_changing_at TIMESTAMP NULL DEFAULT NULL
...
(
  battlefield_id int not null auto_increment,
  effects_allies text
  comment '연합군 전체 효과 
...
{ legion_id: {effect_id:value, ...}, ...}',
  ...
  created_at TIMESTAMP NULL DEFAULT NULL COMMENT '전장이 생성된 시간',
  willbe_rebuilt_at TIMESTAMP NULL DEFAULT NULL
  comment '다음 전장을 구성할 시간',
  hotspots_willbe_rebuilt_at TIMESTAMP NULL DEFAULT NULL
  comment '활성화 지역을 다시 구성할 시간',
  primary key (battlefield_id)
)
;
...
alter table tile add unique battlefield_id__position (battlefield_id, position)
;
...
',
  ...
  training_at TIMESTAMP NULL DEFAULT NULL
...
5: 장비중.(장교 소유) owner: officer_id',
  ...
  level int
  comment '아이템 레벨',
  willbe_made_at TIMESTAMP NULL DEFAULT NULL
...
create unique index idx__battlefield_id__tile_name__gid on tile_scores_general (battlefield_id, tile_name,general_id)
;
...
create unique index idx__battlefield_id__tile_name__lid on tile_scores_legion (battlefield_id, tile_name,legion_id)
;
...
(
	...
	user_id Int,
	dev_uuid varchar(255),
	PRIMARY KEY (mt_id),
	FOREIGN KEY (user_id) REFERENCES user (user_id) ON DELETE CASCADE ON UPDATE CASCADE
);
alter table maintenance_allowed add unique dev_uuid (dev_uuid);

-- Table chat_badword
CREATE TABLE chat_badword 
(
...
alter table chat_badword add unique badword (badword);
INSERT IGNORE INTO chat_badword (created_at, badword) VALUES (NOW(), 'fuck'), (NOW(), 'bitch'), (NOW(), 'shit');

-- Table mail_repo_public
CREATE TABLE mail_repo_public
(
	repo_id Int NOT NULL AUTO_INCREMENT,
	market_type Tinyint NOT NULL DEFAULT 0
	COMMENT '0: all markets
1: google, 2: tstore, 3: olleh, 4: ustore, 5: appstore
',
	recv_force TINYINT NOT NULL
	COMMENT '1: allies, 2: neutral, 3: empire, NOTE THAT This is not equal to common values of enumerating force',
	pull_after_at TIMESTAMP NULL DEFAULT NULL,
	title Text NOT NULL,
	body Text NOT NULL,
	gifts Text,
...
;
CREATE INDEX idx__market_type__recv_force__pull_after_at ON mail_repo_public (market_type, recv_force, pull_after_at);

-- Table mail_repo_public_pulled
CREATE TABLE mail_repo_public_pulled
(
...
-- Table mail_repo_legion
CREATE TABLE mail_repo_legion
(
	repo_id Int NOT NULL AUTO_INCREMENT,
	legion_id INT NOT NULL,
	pull_after_at TIMESTAMP NULL DEFAULT NULL,
	...
	gifts Text,
	pull_count INT NOT NULL DEFAULT 0,
	FOREIGN KEY (legion_id) REFERENCES legion (legion_id) ON DELETE CASCADE ON UPDATE CASCADE,
	PRIMARY KEY (repo_id)
)
;
CREATE INDEX idx__legion_id__pull_after_at ON mail_repo_legion (legion_id, pull_after_at);

-- Table mail_repo_legion_pulled
CREATE TABLE mail_repo_legion_pulled
(
	repo_id INT NOT NULL,
	user_id INT NOT NULL,
	PRIMARY KEY(repo_id, user_id)
...
-- Table mail_repo_user
CREATE TABLE mail_repo_user
(
	repo_id Int NOT NULL AUTO_INCREMENT,
	user_id INT NOT NULL,
	pull_after_at TIMESTAMP NULL DEFAULT NULL,
...
;
CREATE INDEX idx__user_id__pull_after_at ON mail_repo_user (user_id, pull_after_at);


-- Table notice_repo
CREATE TABLE notice_repo
(
	notice_id INT NOT NULL AUTO_INCREMENT,
	notice_type TINYINT NOT NULL DEFAULT 1 COMMENT '1: system, 2: event',
	market_type Tinyint NOT NULL DEFAULT 0
	COMMENT 'recv market_type, 
0: all markets, 1: google, 2: tstore, 3: olleh, 4: ustore, 5: appstore
',
	recv_force TINYINT NOT NULL
	COMMENT '1: allies, 2: neutral, 3: empire, NOTE THAT This is not equal to common values of enumerating force',
	available_after_at TIMESTAMP NULL DEFAULT NULL,
	available_before_at TIMESTAMP NULL DEFAULT NULL,	
	body Text NOT NULL,
	PRIMARY KEY (notice_id)
)
;
CREATE INDEX idx__market_type__recv_force__av_after_at__av_before_at ON notice_repo (market_type, recv_force, available_after_at, available_before_at);


-- Table coupon_repo
CREATE TABLE coupon_repo
(
	coupon_id Int NOT NULL AUTO_INCREMENT,
	code VARCHAR(20) NOT NULL,
	redeem_user_id INT NULL DEFAULT NULL,
	pull_after_at TIMESTAMP NULL DEFAULT NULL,
	title Text NOT NULL,
	body Text NOT NULL,
	gifts Text,
	FOREIGN KEY (redeem_user_id) REFERENCES user (user_id) ON DELETE CASCADE ON UPDATE CASCADE,
	PRIMARY KEY (coupon_id)
)
;
CREATE UNIQUE INDEX idx__code ON coupon_repo (code);
